...
- **No `.env` files committed.** Use `.env.example` with placeholder values. Add `.env*`, `*.pem`, `*.key` to `.gitignore`. See `skills/security.md` for the full secrets policy and what to do if a secret is committed by accident.
